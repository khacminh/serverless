...
module.exports = {
  ...
  async packageFunction(functionName) {
    ...
    const prefix = (!functionObject.package || !functionObject.package.path) ? '' : functionObject.package.path.replace(/^\/+/g, '').replace(/\/+$/g, '');

    const filePaths = await this.resolveFilePathsFunction(functionName);
    const artifactPath = await this.zipFiles(filePaths, zipFileName, prefix);
